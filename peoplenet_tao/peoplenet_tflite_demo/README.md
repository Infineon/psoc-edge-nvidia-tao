...
- **DetectNetV2 model:** Main object detection network (ResNet34-based)
- **NMS model:** Non-maximum suppression to filter overlapping detections
- **Post-processing model:** Converts raw model outputs to properly formatted bounding boxes
- **People detector**: Main application that orchestrates the detection pipeline
...
The NMS model filters overlapping detections based on IoU (Intersection over Union) and confidence thresholds. It is implemented as a separate TensorFlow Lite model for optimal performance.
...
1. Train your model using the NVIDIA TAO Toolkit
...
- Enable Ethos-U acceleration when available
- Adjust the detection thresholds for your specific use case
- Consider reducing the camera resolution for higher FPS
...
- **"Failed to import TensorFlow Lite"**
...
- **"Ethos-U delegate library not found"**
